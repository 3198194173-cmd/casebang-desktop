...
- 两个同企业测试账号：A 首次登录选择“上游建表”，B 首次登录选择“下游建档”；都要在应用可见范围内，并实际登录软件后才进入对应业务。角色会与钉钉账号绑定，测试时不要用同一账号兼任两端。
...
- 下游账号在“建档任务”点击“接收并开始处理”后，软件自动创建按任务和修订隔离的本机工作副本，并调用 Excel/WPS 打开；“下载工作簿”不再作为用户步骤。处理状态下可填写明确原因退回。完成处理并返回原账号仍需先建立编码和图档核验记录。
- 桌面端真实登录已通过；账号现为全软件共享状态。本轮新增业务角色数据库迁移，部署后旧登录会要求重新登录并选择角色。使用第二个同企业账号绑定下游角色后，才能开始首轮双机交接验收。
