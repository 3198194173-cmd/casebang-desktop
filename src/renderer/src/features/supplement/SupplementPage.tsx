    ...
    {result && <>
      <section className="panel"><h3>识别 {result.rows.length} 条 · 可生成 {result.matched} 条 · 未匹配 {result.missing} 条 · 冲突 {result.conflict} 条</h3>
        ...
        {variants.map((v, i) => <fieldset className="supplement-variant" disabled={busy} key={v.variant}><legend>{v.variant || '普通款'} · {matched.filter(r => key(r.variant) === key(v.variant)).length} 条</legend><div className="supplement-form-grid"><label>目标机型<input readOnly value={targetModel + v.variant} /></label>
          {(['domesticPrice', 'overseasPrice'] as const).map((field, f) => <label key={field}>{['建议零售价（元）', '海外零售价（美元）'][f]}<input inputMode="decimal" value={v[field] ?? ''} placeholder="留空保留参考值" onChange={e => setVariants(current => current.map((item, n) => n === i ? { ...item, [field]: e.target.value } : item))} /></label>)}
        </div></fieldset>)}
        <button className="primary-button" disabled={busy || !matched.length} onClick={() => void run(async () => { const path = await desktopApi.supplement.export({ variants, draft: { result, sourcePaths: [masterPath, inputPath] } }); if (path) setMessage(`已导出：${path}`) })}>导出当前工作簿</button>
      </section>
