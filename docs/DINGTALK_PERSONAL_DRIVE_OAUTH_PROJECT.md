...
> 图档核验的最新流程以 [ARTWORK_COMPARISON_PROJECT.md](./ARTWORK_COMPARISON_PROJECT.md) 为准：系列目录按共享工作簿直接选择，随后选择产品类别和样本机型，再对 PDF 预览与图片表截图进行 AI/人工核验。本文中早期“固定父目录”描述属于历史设计，实施时不得作为新建表加工的前置条件。
...
完成定义：当前下游账号完成用户授权后，可以在每个共享工作簿中直接绑定对应系列目录；系统不下载整个目录，能够读取目录元数据、选择类别和样本机型、按名称筛选 PDF 候选、按需取得单页预览或在线预览，并在授权失效时明确提示重新授权。
...
- 系列目录索引按账号和目录节点保存；当前类别、样本机型和核验结果按共享工作簿保存。
