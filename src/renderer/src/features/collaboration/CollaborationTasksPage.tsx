...
export function CollaborationTasksPage({ enabled }: { enabled: boolean }): React.JSX.Element {
  ...
  return <div className="collaboration-page">
    <header className="collaboration-hero"><div><h2>协同工作簿</h2><p>管理系列工作簿，并查看软件加工与 WPS 手动修改记录。</p></div><button className="secondary-button" disabled={busy} onClick={() => void load()}>{busy ? '刷新中…' : '刷新记录'}</button></header>
    ...
    <section className="collaboration-list">
      {!busy && !visibleItems.length ? <div className="collaboration-empty"><strong>当前分类暂无记录</strong><p>完成和作废的工作簿会保留文件与时间轴，但不再占用进行中列表。</p></div> : visibleItems.map(item => <article key={item.id} className="collaboration-task-card">
        ...
        <details className="collaboration-timeline"><summary>编辑时间轴 · {item.activities.length} 条</summary>{item.activities.length ? item.activities.map(activity => <div key={activity.id}><span className={activity.kind}>{activity.kind === 'manual' ? '手动修改' : '软件加工'}</span><p>{activity.actor.displayName}<time>{formatEditTime(activity.occurredAt)}</time></p></div>) : <small>暂无编辑记录</small>}</details>
      </article>)}
    </section>
  </div>
}
