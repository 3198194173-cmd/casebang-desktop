...
describe('database migration files', () => {
  it('are ordered, transaction-runner compatible, and contain the central ledger safeguards', async () => {
    ...
    expect(names).toEqual(['001_foundation.sql', '002_stream_events.sql', '003_dingtalk_auth.sql', '004_business_roles.sql', '005_weboffice.sql', '006_relaxed_workbook_participants.sql', '007_personal_artwork_sources.sql', '008_artwork_work_targets.sql', '009_dingtalk_user_grants.sql', '010_artwork_series_sources.sql', '011_artwork_target_selection.sql'])
    ...
    expect(sql).toContain('CREATE TABLE artwork_sources')
    expect(sql).toContain('UNIQUE(organization_id,user_id)')
    expect(sql).toContain('CREATE TABLE artwork_work_targets')
    expect(sql).toContain('artwork_sources_folder_key')
  })
})
