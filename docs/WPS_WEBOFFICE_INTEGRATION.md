...
1. “新系列建表”“系列补产品”“产品补机型”在最终步骤直接选择“建立共享工作簿”。桌面端先按 Excel 实际显示尺寸优化并复用嵌入图片，再以可重试的 64 KB POST 分块上传中央服务；服务器合并后校验整文件 SHA-256，桌面端随后自动清理临时文件，不要求用户先导出本地副本。
2. “导出本地副本”是独立的可选操作，仅用于离线备份或发送；本地副本不与中央工作簿双向同步，也不是进入协作流程的前置条件。
3. 中央服务建立唯一工作簿记录和修订 1，文件保存在私有存储；同一文件重复点击不会建立第二条记录。没有第二账号时由当前账号同时作为创建人与处理参与人，不阻塞建档。
4. 登录时选择的上游/下游身份只切换业务工具，不限制共享工作簿编辑。当前账号退出后可重新选择业务端登录；有第二账号时仍自动关联最近登录的下游账号。
...
6. 保存完成后，“工作簿记录”显示最新修订号、实际编辑人和编辑时间；阶段仍由人员明确选择，不根据一次自动保存误判业务已经完成。
...
3. 上游账号点击“建立共享工作簿”，上下游分别点击“WPS 在线编辑”，确认两边进入相同 `file_id`。
