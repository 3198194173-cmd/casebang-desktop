...
export function MaterialDataPage({ enabled }: { enabled: boolean }): React.JSX.Element {
  ...
  return <div className="material-data-page">
    ...
    {tab === 'master' ? <section className="md-panel">
      ...
      <section className="md-panel md-artwork-source"><header><div><span className="eyebrow">DINGTALK DRIVE</span><h3>系列印刷图档</h3><p>系列图档不再绑定固定父目录。请进入“新建表加工”，选择共享工作表后，直接绑定当前系列文件夹，再选择产品类别和样本机型进行 PDF 图案核验。</p></div></header><div className="md-artwork-empty"><strong>按工作簿绑定系列目录</strong><p>每个系列使用自己的钉盘文件夹链接；目录索引、PDF 文件名解析、图片表匹配和 AI 图案核验均保存在对应工作簿下。切换下游账号后只读取该账号自己的授权数据。</p></div></section>}
  </div>
}
