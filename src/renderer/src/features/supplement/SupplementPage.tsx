  ...
  return <div className="stack-xl supplement-page">
    <section className="panel supplement-setup"><div className="supplement-heading"><div><span className="eyebrow">新机型补充</span><h2>原有产品补充新机型</h2><p>用补齐表中的原物料名称检索总表，再生成目标机型的新记录。</p></div><span className={`supplement-ready ${masterPath ? 'ready' : ''}`}>{masterPath ? '总表已就绪' : '缺少总物料表'}</span></div>
      ...
      <div className="supplement-run-row"><div><strong>准备完成后开始识别</strong><small>不会修改总物料表和补齐表</small></div><button className="primary-button" disabled={busy || !masterPath || !inputPath || !targetModel.trim()} onClick={() => void run(async () => {
        setResult(null); setVariants([]); setExportedPath(''); setPublishedRecordId('')
    ...
    {result && <>
      <section className="panel"><h3>识别 {result.rows.length} 条 · 可生成 {result.matched} 条 · 未匹配 {result.missing} 条 · 冲突 {result.conflict} 条</h3>
        ...
        {variants.map((v, i) => <fieldset className="supplement-variant" disabled={busy} key={v.variant}><legend>{v.variant || '普通款'} · {matched.filter(r => key(r.variant) === key(v.variant)).length} 条</legend><div className="supplement-form-grid"><label>目标机型<input readOnly value={targetModel + v.variant} /></label>
          {(['domesticPrice', 'overseasPrice'] as const).map((field, f) => <label key={field}>{['建议零售价（元）', '海外零售价（美元）'][f]}<input inputMode="decimal" value={v[field] ?? ''} placeholder="留空保留参考值" onChange={e => { setVariants(current => current.map((item, n) => n === i ? { ...item, [field]: e.target.value } : item)); setExportedPath(''); setPublishedRecordId('') }} /></label>)}
        </div></fieldset>)}
        <div className="supplement-final-actions">
          <button className="primary-button" disabled={busy || !matched.length} onClick={() => void run(async () => { const path = await desktopApi.supplement.export({ variants, draft: { result, sourcePaths: [masterPath, inputPath] } }); if (path) { setExportedPath(path); setPublishedRecordId(''); setMessage(`已导出：${path}`) } })}>导出当前工作簿</button>
          <button className="primary-button" disabled={busy || !exportedPath || Boolean(publishedRecordId)} onClick={() => void run(async () => { const fileName = exportedPath.split(/[\\/]/).pop() || `产品补机型-${targetModel}.xlsx`; const response = await desktopApi.collaboration.publishWorkbook({ path: exportedPath, title: fileName, sourceWorkflow: 'new-models' }); setPublishedRecordId(response.item.id); setMessage(response.duplicate ? '该工作簿已在共享流程中。' : '已导入共享工作簿，等待下游加工。') })}>{publishedRecordId ? '已进入共享流程' : '导入共享工作簿'}</button>
        </div>
        {!exportedPath && <p>请先导出当前工作簿，再将该文件导入共享流程。</p>}
      </section>
