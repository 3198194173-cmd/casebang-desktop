...
export function MaterialDataPage({ enabled }: { enabled: boolean }): React.JSX.Element {
  ...
  const bindArtworkSource = async (): Promise<void> => {
    ...
    setMessage(`已为当前账号绑定固定父目录“${source.folderName}”，索引 ${source.fileCount} 个文件。`)
  }
  ...
  return <div className="material-data-page">
    ...
    {tab === 'master' ? <section className="md-panel">
      ...
      <section className="md-panel md-artwork-source"><header><div><span className="eyebrow">DINGTALK DRIVE</span><h3>固定印刷图档父目录</h3><p>这里只绑定第一个长期不变的组织空间父目录；每个系列自己的文件夹在“新建表加工 → 核验印刷图档”中选择。</p></div></header>
        <form className="md-artwork-bind" onSubmit={event => { event.preventDefault(); void run(bindArtworkSource) }}><label>固定父目录链接<input value={artworkUrl} onChange={event => setArtworkUrl(event.target.value)} placeholder="例如：https://alidocs.dingtalk.com/i/desktop/folders/..." /></label><button className="primary" disabled={busy || !artworkUrl.trim()}>{artworkSource ? '更换固定父目录' : '绑定固定父目录'}</button></form>
        {artworkSource ? <><div className="md-artwork-summary"><div><small>固定父目录</small><strong>{artworkSource.folderName}</strong></div><div><small>文件</small><strong>{artworkSource.fileCount}</strong></div><div><small>系列与子目录</small><strong>{artworkSource.folderCount}</strong></div><div><small>最近同步</small><strong>{formatDateTime(artworkSource.indexedAt)}</strong></div><button disabled={busy} onClick={() => void run(syncArtworkSource)}>同步父目录索引</button></div>
